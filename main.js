...
underlineToggle.addEventListener("change", (e) => {
  underlineEnabled = e.target.checked;

  checkboxLabel.textContent = underlineEnabled
    ? "Disable Underline"
    : "Enable Underline";
});
...
async function deghost() {
  ...
  for (const unit of units) {
    ...
    // todo the longest duration is the total duration for the de0ghost effect
    // investigate how to have these happen independently
    if (underlineEnabled) {
      unit.classList.add("underlined");
      await sleep(UNDERLINE_INTERVAL);
      unit.classList.remove("underlined");
    }
